...
//		mouseReleased();
	...
	private void addKey() {	
		...
		text("Size~~ Magnitude", 65, 180);
		
		text("Shallow", 75, 205); 
		text("Intermediate", 75, 230);
		text("Deep", 75, 255);
		text("Past Hour", 75, 280);
		
				
		fill(137, 5, 47);
		triangle(40, 110, 50, 92, 60, 110);
...
//					System.out.println(isInCountry(earthquake,m));
	...
	private void printQuakes() 
	{
		...
		//        
		//      Inside the loop, first initialize a quake counter.  להגדיר פרופרטי?
...
int oceanCounter=0;
	...
	//created a hashmap for storing each land markers -country name
HashMap<String,Integer>hm =new HashMap<>();

	for (Marker quake :quakeMarkers) {
		//if the value is ocean earthquake , we only count their amount with running counter
		if (quake  instanceof OceanQuakeMarker) {
				oceanCounter++;			
			}
		//else, if the earthquake is on lane ,
		//we get the country's name , if the name already on our list , we add to its counter +1 , 
		//if not, we add the country with key value 1
	else if (quake  instanceof LandQuakeMarker){
				landCounter++;
				String name =(String)quake.getProperty("country");
//				System.out.println(name);    /* for checking the country's name */
	...
	// printing each value in our hm hashMap .
	if (hm.isEmpty()) {
		System.out.println("the list is Empty ");
	}
	else {	
		for (Entry<String, Integer> entry : hm.entrySet()) { 
         System.out.println("Country(Key) = " + entry.getKey() + 
                          ", Number(Value) = " + entry.getValue());
		}
		System.out.println( "Ocean :"+oceanCounter+" land : "+landCounter);
	}
}
...
//		System.out.println("added "+"country 1 :"+ country.getProperty("name"));
